...
class TestGetOrders:
    ...
    @allure.description('Отправляем запрос на получение заказов без передачи токена,\
                        проверяем что код ответа 401 и текст  ответа "You should be authorised"')
    ...
    @allure.description('Создаем пользователя, создаем заказ,\
                        oтправляем запрос на получение заказов с токеном авторизации, \
